...
var clickHandler = 'tap';
  ...
  $(".TouchPoint").bind(clickHandler, function(e) {
    ...
    $('#cia-btn1,#cia-btn1_d').on(clickHandler, function() {
        if(!$(".child2tab_1").is(':visible')) {
            localStorage.setItem("page5child2", true);
        }
        //return false;
        localStorage.setItem("goToSlide", 3 - 1);
        parent.loadPageFromChildPage(5);
    })
    $('#cia-btn2,#cia-btn2_d').on(clickHandler, function() {
    ...
    //ckd-btn2
    $('#ckd-btn2').on(clickHandler, function(){
        localStorage.setItem("goToSlide",2);
        parent.loadPageFromChildPage(5);
    })


// LOCALHOST WORKS HERE
    if (localStorage.goToSlide == 2 - 1) {
        myswiper.slideTo(1, 0)
        localStorage.removeItem('goToSlide')
    }

    if (localStorage.goToSlide == 2 - 4) {
